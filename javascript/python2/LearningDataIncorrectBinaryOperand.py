...
operator_embedding_size = 200  # the number should correspond to the maximum
...
class LearningData(object):
    ...
    def code_to_xy_pairs(self, gen_negatives, bin_op, xs, ys, name_to_vector, type_to_vector, node_type_to_vector, code_pieces):
        ...
        left_type_vector = pad_sequences(left_type_vector, maxlen=200, padding='post')

        right_type_vector = [type_to_vector.get(right_type, [0]*type_embedding_size)]

        right_type_vector = pad_sequences(right_type_vector, maxlen=200, padding='post')
        ...
        x_correct=[]
        x_correct.append(left_vector)
        x_correct.append(right_vector)
        x_correct.append(operator_vector)
        x_correct.append(left_type_vector)
        x_correct.append(right_type_vector)
        x_correct.append(parent_vector)
        x_correct.append(grand_parent_vector)
        y_correct = [0]
        xs.append(x_correct)
        ys.append(y_correct)
        ...
        if gen_negatives:
            ...
            if replace_left:
                ...
                x_incorrect=[]
                x_incorrect.append(other_operand_vector)
                x_incorrect.append(right_vector)
                x_incorrect.append(operator_vector)
                x_incorrect.append(other_operand_type_vector)
                x_incorrect.append(right_type_vector)
                x_incorrect.append(parent_vector)
                x_incorrect.append(grand_parent_vector)
                ...
                #    parent_vector + grand_parent_vector
                x_incorrect=[]
                x_incorrect.append(left_vector)
                x_incorrect.append(other_operand_vector)
                x_incorrect.append(operator_vector)
                x_incorrect.append(right_type_vector)
                x_incorrect.append(other_operand_type_vector)
